...
interface Props {
  ...
  onDuplicateRow?: (row: ExportRow, index: number) => void;
...
export const TABLE_COLUMNS: ColumnDef[] = [
  ...
  { key: 'status', label: 'Actions', defaultWidth: 105, minWidth: 85, align: 'right' },
];
...
export function CsvTablePreview({ rows, onUpdateRow, onDuplicateRow, onRemoveRow }: Props) {
  ...
  // Copy specific cell value (e.g. part number or item_id)
              ...
              {filteredRows.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 font-sans font-semibold text-slate-800 overflow-hidden group/part">
                      <div className="flex items-center justify-between gap-1.5 truncate">
                        ...
                        <button
                          type="button"
                          onClick={() => handleCopyText(row.inputPart, `part-${row.id}`)}
                          title="Copy Part Name"
                          className="opacity-0 group-hover/part:opacity-100 hover:opacity-100 p-0.5 text-slate-400 hover:text-indigo-600 rounded transition-opacity cursor-pointer shrink-0"
                        >
                          {copiedId === `part-${row.id}` ? (
                    ...
                    {/* item_id (Internal ID) */}
                    <td className="px-3 py-2.5 font-bold bg-indigo-50/30 overflow-hidden group/id">
                      {isEditing ? (
                        <input
                          type="text"
                          value={row.item_id}
                        ...
                        <div className="flex items-center justify-between gap-1 truncate">
                          <span className="text-indigo-900 font-bold bg-indigo-100/60 px-1.5 py-0.5 rounded truncate">
                            {row.item_id}
                          </span>
                          <button
                            type="button"
                            onClick={() => handleCopyText(row.item_id, `id-${row.id}`)}
                            title="Copy Internal ID"
                            className="opacity-0 group-hover/id:opacity-100 hover:opacity-100 p-0.5 text-indigo-500 hover:text-indigo-700 rounded transition-opacity cursor-pointer shrink-0"
                          >
                            {copiedId === `id-${row.id}` ? (
                              <Check className="w-3 h-3 text-green-600 font-bold" />
                            ) : (
                    ...
                    <td className="px-3 py-2.5 text-right font-sans overflow-hidden">
                      <div className="flex items-center justify-end gap-1.5">
                        {/* Duplicate Line Icon Button */}
                        <button
                          type="button"
                          onClick={() => {
                            onDuplicateRow?.(row, idx);
                            setDuplicatedId(row.id);
                            setTimeout(() => setDuplicatedId(null), 1500);
                          }}
                          title={duplicatedId === row.id ? 'Duplicated!' : 'Duplicate this line'}
                        ...
                        {onRemoveRow && (
                          <button
                            ...
                            className="p-1 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded transition-colors cursor-pointer shrink-0"
                          >
